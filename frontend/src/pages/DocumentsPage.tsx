...
function AddDocForm({ category, onSave, onCancel }: {
  ...
  const submit = async () => {
    if (!name.trim()) return;
    setSaving(true);
    await onSave({ name: name.trim(), category, fileType, url: url.trim() || undefined, description: description.trim() || undefined, tags: tags.trim() || undefined });
    setSaving(false);
  };
...
function EditModal({ doc, onSave, onClose }: {
  ...
  const submit = async () => {
    if (!name.trim()) return;
    setSaving(true);
    await onSave(doc.id, { name: name.trim(), fileType, url: url.trim() || undefined, description: description.trim() || undefined, tags: tags.trim() || undefined });
    setSaving(false);
    onClose();
  };
