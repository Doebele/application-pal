...
**LetterDraftPanel** (`DetailDrawer.tsx`, `preparing_letter` stage): the **single guided workspace** for cover-letter creation — a numbered 5-step flow. It fully owns the stage: `STAGE_TILES.preparing_letter` is `[]` and `StageAiActions` renders nothing for this stage (the two-column tasks/actions grid collapses to one column). Steps: **1** Individuelle Angaben (free text, top — also feeds the opening sentence, persisted via `letter-inputs.jobNotes`); **2** Bausteine (`POST .../ai/letter-angles` → Berührungspunkte / Werte-Match / Nutzenargumente, checkboxes, editable); **3** Eröffnungssatz (`POST .../ai/opening-sentences` with `letterInputs.{angles,jobNotes}` so the 3 sentences reference the selection — radio pick, shows Ansatz/Begründung/Empfehlung, stored in `letter-inputs.openingSentence`); **4** „Anschreiben erstellen" (`cover-letter` with `letterInputs.{angles,jobNotes,openingSentence}`); **5** Ergebnis inline with Kopieren / Als Google Doc / Review (`letter-review`) / Anpassen (regenerate with a hint via `additionalContext`). Copy/Google-Doc appear **only after** a letter exists. Every generation appends a version via `POST .../ai/letter-versions/save`; the expanded panel shows a right-hand version sidebar — click restores a version via `POST .../ai/letter-versions/restore` (writes it back to `aiResultsCache["cover-letter"]`). All selections persist through `PATCH .../letter-inputs` into `aiResultsCache` (survives tab switches). All AI calls send `language` in the body so the correspondence-language toggle is respected without a race.
