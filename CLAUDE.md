...
**`langPrompt()` in every AI system prompt**: All AI endpoints prepend `langPrompt(lang)` to the system prompt — a forceful "write ALL output strictly in this language regardless of the instructions below" directive (the otherwise-German prompt bodies would otherwise drag output to German). Default is `"de"`. The letter endpoints (`cover-letter`, `letter-angles`, `opening-sentences`, `letter-review`) resolve `lang = bodyLang ?? app_.language ?? "de"` — i.e. a `language` field in the request body wins, so the DE/EN toggle takes effect immediately without a DB round-trip race. Never hardcode a language in a system prompt.
...
| `POST /api/applications/:id/ai/cover-letter/humanize` | Post-pass: rewrites an existing letter (`{ subject, body }`, or cached) through the language humanizer ruleset only — no fact/structure changes; returns `{ subject, body }` (not persisted — frontend saves a new version). Always applies the rules (explicit action, ignores the `humanize` toggle) |
| `POST /api/applications/:id/ai/letter-angles` | `{ beruehrungspunkte, werteMatch, nutzenArgumente }` — job-specific building-block suggestions; accepts `language`; persisted to `aiResultsCache["letter-angles"]` |
| `POST /api/applications/:id/ai/opening-sentences` | `{ saetze: [{satz, ansatz, erklaerung, empfehlung}] }` — accepts `language` + `letterInputs.{angles,jobNotes}` so sentences reference the selection; persisted to `aiResultsCache["opening-sentences"]` |
| `PATCH /api/applications/:id/letter-inputs` | Persists `LetterDraftPanel` selection (not AI-generated) into `aiResultsCache["letter-inputs"]`; body `{ selectedAngles?: string[], jobNotes?: string, openingSentence?: string }` |
| `POST /api/applications/:id/ai/letter-versions/save` | Appends `{ subject, body, createdAt }` to `aiResultsCache["letter-versions"].versions`; returns the list. Called only by the AI adjust/generate flow — a **new** version each time |
| `POST /api/applications/:id/ai/letter-versions/update` | body `{ index, subject, body }` — overwrites `versions[index]` **in place** (keeps its `createdAt`) and syncs `aiResultsCache["cover-letter"]`; used for inline manual edits of the shown letter (no new version) |
| `POST /api/applications/:id/ai/letter-versions/restore` | body `{ index }` — copies that version into `aiResultsCache["cover-letter"]`, returns it |
...
| `POST /api/applications/:id/ai/letter-review` | `{ gesamteindruck, staerken, verbesserungen, cliches, tonalitaet, laenge, personalisierung, promptVorschlaege }` — `promptVorschlaege` is 2-4 ready-to-paste adjustment instructions, rendered as clickable chips in `LetterDraftPanel`. Auto-reads `aiResultsCache["cover-letter"]` if no `coverLetterContent` provided — persisted |
...
**LetterDraftPanel** (`DetailDrawer.tsx`, `preparing_letter` stage): the **single guided workspace** for cover-letter creation, split into 3 horizontal sub-tabs (`.tab`/`.tab.active`, same underline pattern as the drawer's outer tab bar) to keep any single screen short — each tab shows a green checkmark once it has content:
- **Tab 1 „Vorbereitung"**: Individuelle Angaben (free text — also feeds the opening sentence, persisted via `letter-inputs.jobNotes`) + Bausteine (`POST .../ai/letter-angles` → Berührungspunkte / Werte-Match / Nutzenargumente, checkboxes, editable).
- **Tab 2 „Entwurf"**: Eröffnungssatz (`POST .../ai/opening-sentences` with `letterInputs.{angles,jobNotes}` so the 3 sentences reference the selection — radio pick, shows Ansatz/Begründung/Empfehlung, stored in `letter-inputs.openingSentence`) + „Anschreiben erstellen" button (`cover-letter` with `letterInputs.{angles,jobNotes,openingSentence}`). Clicking it immediately switches to Tab 3, which shows a spinner until the letter arrives.
- **Tab 3 „Ergebnis"**: the letter inline with Kopieren / Als Google Doc / Review (`letter-review`) / Anpassen (regenerate with a hint via `additionalContext`) / **Vermenschlichen** (`humanizeLetter()` → `POST .../ai/cover-letter/humanize`: a post-pass that rewrites the *current* letter through the language humanizer ruleset only — no fact/structure changes — always applies regardless of the `/letter-coach` toggle, saved as a new version) — these actions appear **only after** a letter exists. A „Version N / M" line above the letter shows which stored version is displayed (`currentVersionIdx` = `versions.findIndex(v => v.body === letter.body)`). The review is rendered with the same styling as the old `renderTileContentLarge`/`AiSection`+`BulletList`+`TagBadge` pattern used elsewhere (green Stärken, amber Verbesserungen, red Cliché badges), plus a `promptVorschlaege` list (ready-to-use adjustment instructions from the backend) rendered as clickable chips — clicking one pre-fills the Anpassen textarea via `applySuggestedPrompt()`.

**Review freshness** (`LetterDraftPanel`): a review only describes the exact letter it ran against. `reviewCurrent = reviewEntry.createdAt >= letterEntry.createdAt` — both timestamps come from the `aiResultsRegistry` entries and survive reload (rebuilt from persisted `_savedAt`). Any new/adjusted/restored draft bumps `cover-letter.createdAt`, so the review auto-flags stale. The review block shows a green „Bezieht sich auf den aktuellen Entwurf (Version N)" banner when fresh, amber „…früheren Entwurf — bitte neu prüfen" when stale. The Review button is `disabled` while `reviewCurrent` (nothing to re-check; tooltip „Review ist aktuell") and relabels to „Review aktualisieren" while a stale review is shown — no separate state, no backend change.

The result letter is **inline-editable** (subject `<input>` + body `AutoTextarea`); edits save on blur via `POST .../ai/letter-versions/update`, overwriting the **current** version in place — a new version is created **only** by the AI adjust/generate flow (`.../save`). The review's `promptVorschlaege` chips **accumulate** into the Anpassen textarea (one instruction per line): clicking a chip toggles it in/out, and applied chips are marked (accent border + Check icon), derived from the textarea text via `isSuggestionApplied()`.

Every AI generation appends a version via `POST .../ai/letter-versions/save`; the expanded panel shows a right-hand version sidebar — click restores a version via `POST .../ai/letter-versions/restore` (writes it back to `aiResultsCache["cover-letter"]`). All selections persist through `PATCH .../letter-inputs` into `aiResultsCache` (survives tab switches). All AI calls send `language` in the body so the correspondence-language toggle is respected without a race. `STAGE_TILES.preparing_letter` is `[]` and `StageAiActions` renders nothing for this stage (the two-column tasks/actions grid collapses to one column) — the panel fully owns the stage.

**Cross-tile AI-job tracking** (`useUiStore.aiJobs`, in-memory only — `partialize` strips it from persistence): `startAiJob(appId, kind)` / `finishAiJob(appId, kind)` / `markAiJobsSeen(appId)` track in-flight cover-letter/review generations per application. `LetterDraftPanel` reads `draftRunning`/`reviewRunning` as `local generating flag || persisted job.status === "running"`, so the spinner (letter box overlay, review „…wird erstellt" banner in accent) survives switching applications (the drawer unmounts/remounts). On finish, the job flips to `{ status: "done", seen: false }`; `Card.tsx`'s `AiJobIndicator` shows a spinner while running and an accent ✦ „new result" mark once done-but-unseen, cleared by `markAiJobsSeen` when the user next opens that app's drawer. The fetch is not tied to the component, and completion always calls `queryClient.invalidateQueries` so the DB-persisted result loads on return.

**Letter-Coach page** (`/letter-coach`, `LetterCoachPage.tsx`): standalone Rail nav item where the user configures persistent, reusable cover-letter guidance — 7 free-text fields (structure, values, strengths, phrases, styleRules, noGos, referenceLetter) plus a `humanize` boolean toggle, stored as one JSON blob in `user_profile.letter_config`. Autosaves per-field via `PATCH /api/profile { letterConfig }` (not `PUT` — avoids full-profile validation for a single-field edit). Backend helper `buildLetterGuidance(letterConfigRaw)` formats the non-empty fields into a `## Verbindliche Vorgaben des Kandidaten` prompt block, appended in `cover-letter`, `letter-review`, and `opening-sentences`. Empty fields are never silently defaulted — each has a "Standard übernehmen" button that copies a recommended template into the field on click.

**Humanizer** (`humanizerPrompt(lang, letterConfigRaw)` + `HUMANIZER_GUIDELINES: Record<"de"|"en"|"fr", string>`): appends a language-specific "write like a human, avoid AI tells" ruleset to the system prompt of the AI-*prose* endpoints — `cover-letter`, `letter-review` (as an evaluation criterion: check the letter against the rules + write the critique by them), `opening-sentences`, `letter-angles`, `cv-highlights`, and the `cv-doc` highlight sub-call — keyed by correspondence language. **On by default** (opt-out: `humanize !== false`), toggled on `/letter-coach`. Deliberately NOT applied to the user's Master CV text (facts stay intact) or to job/company-analysis endpoints. Rulesets are original condensed guidance informed by the open-source humanizer skills (blader/humanizer EN, samber/cc-skills humaniseur-fr, LOGIN-TB/claude-skills vermenschlichen DE/MIT). The `humanize` boolean is excluded from `LETTER_GUIDANCE_LABELS`/`DEFAULTS` (string-field maps) on both backend and frontend.
...
**KI-Erkenntnisse (DetailsTab)**: `STAGE_TILES` mapping determines which tile IDs show per stage (note: `preparing_letter` is `[]` — that stage is handled entirely by `LetterDraftPanel`). Click → `TileExpandView`. After AI generation, invalidates `["applications"]` + `["application", appId]` queries.
