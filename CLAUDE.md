...
**Humanizer** (`humanizerPrompt(lang, letterConfigRaw)` + `HUMANIZER_GUIDELINES: Record<"de"|"en"|"fr", string>`): appends a language-specific "write like a human, avoid AI tells" ruleset to the system prompt of the AI-*prose* endpoints — `cover-letter`, `opening-sentences`, `letter-angles`, `cv-highlights`, and the `cv-doc` highlight sub-call — keyed by correspondence language. **On by default** (opt-out: `humanize !== false`), toggled on `/letter-coach`. Deliberately NOT applied to the user's Master CV text (facts stay intact) or to job/company-analysis endpoints. Rulesets are original condensed guidance informed by the open-source humanizer skills (blader/humanizer EN, samber/cc-skills humaniseur-fr, LOGIN-TB/claude-skills vermenschlichen DE/MIT). The `humanize` boolean is excluded from `LETTER_GUIDANCE_LABELS`/`DEFAULTS` (string-field maps) on both backend and frontend.
